...
<link href="<%=request.getContextPath()%>/resources/css/main/board/qna.css" rel="stylesheet">
<!DOCTYPE html>
<html lang="kor">
...
<div class="body_wrap">
    <div class="qna_wrap">
        ...
        <div class="qna_table">
            <table>
                ...
                <c:forEach var="item" items="${qnaDto}">
                    <tr>
                        <td>${item.qnaId}</td>
                        <td class="w600"><a href="/board/qnaOne?qnaId=${item.qnaId}">${item.qnaTitle}</a></td>
                        <td>방수현</td>
                        <td><fmt:formatDate value="${item.qnaRdate}" pattern="yyyy-MM-dd"/></td>
                        <td>${item.qnaHits}</td>
                    </tr>
                </c:forEach>
                </tbody>
    ...
    <div class="qna_write">
        <button type="submit" onclick="location.href='/board/insertQna'">글 쓰기</button>
    </div>
